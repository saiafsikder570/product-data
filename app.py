...
st.title("🍱 Super Tasty Food - প্রো ড্যাশবোর্ড")
...
# সেশন স্টেট ইনিশিয়ালাইজ করা (এরর এড়ানোর জন্য)
...
# ৬. ফলাফল প্রদর্শন (শুধুমাত্র যদি ডাটা প্রসেসড থাকে)
if st.session_state.df_final is not None and st.session_state.summary is not None:
    ...
    def create_pdf():
        ...
        pdf.cell(200, 10, txt=f"Total Revenue: BDT {s['rev']:,.0f}", ln=True)
        pdf.cell(200, 10, txt=f"Net Profit: BDT {s['net']:,.0f}", ln=True)
        return pdf.output(dest='S').encode('latin-1', 'ignore')

    st.subheader("📥 ডাউনলোড করুন")
    pdf_file = create_pdf()
    st.download_button(label="📄 পিডিএফ রিপোর্ট ডাউনলোড", data=pdf_file, file_name="STF_Report.pdf", mime="application/pdf")
